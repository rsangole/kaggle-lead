...
dat2[, cloud_coverage_missing := as.factor(cloud_coverage == 255)]
dat2[, year_built_missing := as.factor(year_built == 255)]
dat2[, meter_reading_missing := as.factor(meter_reading_missing)]
dat2[, primary_use := as.factor(primary_use)]
int_to_dbl_cols <- dat2[, names(.SD), .SDcols = is.numeric]
dat2[, (int_to_dbl_cols) := lapply(.SD, as.double), .SDcols = int_to_dbl_cols]
...
po_pca_temperature %>>%
    ...
    # po("imputesample") %>>%
    po(lrn_catboost) |>
    as_learner() -> lrn_pca_encode_over_catboost
lrn_pca_encode_over_catboost$predict_sets <- c("train", "test")
# lrn_pca_encode_over_catboost$param_set$values <- list(classif.catboost.task_type = "GPU")
...
po_pca_temperature %>>%
    ...
    # po("imputesample") %>>%
    po(lrn_catboost) |>
    as_learner() -> lrn_pca_tgtencode_over_catboost
lrn_pca_tgtencode_over_catboost$predict_sets <- c("train", "test")
# lrn_pca_tgtencode_over_catboost$param_set$values <- list(classif.catboost.task_type = "GPU")
...
po_encode %>>%
    ...
    # po("imputesample") %>>%
    po(lrn_catboost) |>
    as_learner() -> lrn_encode_over_catboost
lrn_encode_over_catboost$predict_sets <- c("train", "test")
# lrn_encode_over_catboost$param_set$values <- list(classif.catboost.task_type = "GPU")
...
#     po("imputesample") %>>%
#     po(lrn_catboost) |>
#     as_learner() -> lrn_pca_encode_under_catboost
# lrn_pca_encode_under_catboost$predict_sets <- c("train", "test")
...
#     po("imputesample") %>>%
#     po(lrn_catboost) |>
#     as_learner() -> lrn_pca_over_catboost
# lrn_pca_over_catboost$predict_sets <- c("train", "test")
...
#     po("imputesample") %>>%
#     po(lrn_catboost) |>
#     as_learner() -> lrn_over_catboost
# lrn_over_catboost$predict_sets <- c("train", "test")
...
design3 <- benchmark_grid(
    ...
    learners = list(
        ...
        # lrn_pca_encode_under_catboost,
        # lrn_pca_over_catboost,
        # lrn_over_catboost,
...
bmr3$aggregate(measure) -> bmr_perf3
bmr_perf3[, learner_id := forcats::fct_reorder(
    learner_id, auc_test
)]
bmr_perf3
dotplot(
...
qs::qsave(design3, "data/results/stratified-sampling-approach/design3.qs")
qs::qsave(bmr3, "data/results/stratified-sampling-approach/bmr3.qs")
qs::qsave(bmr_perf3, "data/results/stratified-sampling-approach/bmr_perf3.qs")
